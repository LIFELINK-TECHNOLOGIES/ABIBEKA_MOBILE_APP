  ...
  return (
    <View>
      <Svg width={CW} height={CH}>
        ...
        <Path d={line} fill="none" stroke={B.primary} strokeWidth={1.8} strokeLinecap="round" />
        {pts.map((p, i) => (
          <React.Fragment key={i}>
            <Circle cx={p.x} cy={p.y} r={5}   fill={B.card} />
            <Circle cx={p.x} cy={p.y} r={3.5} fill={p.v >= 60 ? B.accent : p.v >= 50 ? B.amber : B.red} />
          </React.Fragment>
        ))}
      </Svg>
      <View style={{ flexDirection: "row", justifyContent: "space-between", paddingHorizontal: PAD, marginTop: 6 }}>
        {DAY_LABELS.map((d, i) => (
          <Text key={i} style={{ fontSize: 10, color: B.muted2, textAlign: "center", flex: 1 }}>{d}</Text>
        ))}
      </View>
    </View>
  );
...
// ─── Emotion donut ────────────────────────────────────────────────────────────
const EmotionWheel = () => {
  const SIZE = 130, CX = 65, CY = 65, R = 44, SW = 16;
  let cum = 0;
  const arcs = EMOTIONS_DIST.map(e => {
    ...
    return { ...e, d: `M ${x1} ${y1} A ${R} ${R} 0 ${pct > 0.5 ? 1 : 0} 1 ${x2} ${y2}` };
  });
  ...
  return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 20 }}>
      <Svg width={SIZE} height={SIZE}>
        ...
        <SvgText x={CX} y={CY - 4}  textAnchor="middle" fill={B.text}  fontSize={20} fontWeight="800">34%</SvgText>
        <SvgText x={CX} y={CY + 14} textAnchor="middle" fill={B.muted} fontSize={10}>Stressed</SvgText>
      </Svg>
      <View style={{ flex: 1, gap: 10 }}>
        {EMOTIONS_DIST.map((e, i) => (
          <View key={i} style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
            <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: e.color }} />
            <Text style={{ fontSize: 12, color: B.muted, flex: 1 }}>{e.label}</Text>
            <Text style={{ fontSize: 12, fontWeight: "700", color: e.color }}>{e.pct}%</Text>
          </View>
        ))}
      </View>
    </View>
  ...
  return (
    <View style={{ gap: 14 }}>
      {TOP_ISSUES.map((issue, i) => {
        const w = anims[i].interpolate({ inputRange: [0, 1], outputRange: ["0%", `${issue.pct}%`] });
        const c = issue.pct >= 70 ? B.red : issue.pct >= 50 ? B.amber : B.primary;
        return (
          <View key={i}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
              <Text style={{ fontSize: 13, color: B.text }}>{issue.issue}</Text>
              <Text style={{ fontSize: 12, fontWeight: "700", color: c }}>{issue.pct}%</Text>
            </View>
            <View style={s.issueTrack}>
              <Animated.View style={[s.issueFill, { width: w, backgroundColor: c }]} />
            </View>
          </View>
        );
      })}
  ...
  <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
    ...
    <View style={[s.card, { borderColor: B.primary + "28" }]}>
      <View style={s.orgHeader}>
        <View style={s.orgIconWrap}><Text style={{ fontSize: 26 }}>{ORG.icon}</Text></View>
        <View style={{ flex: 1 }}>
    ...
    <View style={s.alertCard}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.alertSub}>72% of members report elevated stress this week</Text>
        </View>
    ...
    <View style={s.card}>
      ...
      <View style={{ marginTop: 16 }}><WellnessChart /></View>
    </View>
    ...
    <View style={s.card}>
      ...
      <View style={{ marginTop: 18 }}><EmotionWheel /></View>
    </View>
    ...
    <View style={s.card}>
      ...
      <View style={{ marginTop: 18 }}><IssuesList /></View>
    </View>
...
const s = StyleSheet.create({
  ...
  card:        { backgroundColor: B.card, borderRadius: 20, borderWidth: 1, borderColor: B.border, padding: 20 },
  cardTitle:   { fontSize: 15, fontWeight: "800", color: B.text, letterSpacing: -0.2, marginBottom: 3 },
  cardSub:     { fontSize: 12, color: B.muted },
  orgHeader:   { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 16 },
  orgIconWrap: { width: 52, height: 52, borderRadius: 17, backgroundColor: B.primary + "18", borderWidth: 1, borderColor: B.primary + "28", alignItems: "center", justifyContent: "center" },
  orgName:     { fontSize: 17, fontWeight: "800", color: B.text, marginBottom: 3, letterSpacing: -0.3 },
  orgIndustry: { fontSize: 12, color: B.muted },
  memberBadge: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 10, backgroundColor: B.accent + "10", borderWidth: 1, borderColor: B.accent + "25" },
  memberDot:   { width: 6, height: 6, borderRadius: 3, backgroundColor: B.accent },
  ...
  statVal:     { fontSize: 22, fontWeight: "900", textAlign: "center", marginBottom: 3 },
  statLabel:   { fontSize: 10, color: B.muted, textAlign: "center" },
  alertCard:   { paddingVertical: 14, paddingHorizontal: 16, borderRadius: 16, backgroundColor: B.red + "08", borderWidth: 1, borderColor: B.red + "25" },
  alertTitle:  { fontSize: 13, fontWeight: "800", color: B.text, marginBottom: 2 },
  alertSub:    { fontSize: 12, color: B.muted },
  alertBadge:  { fontSize: 9, fontWeight: "800", color: B.red, letterSpacing: 1 },
  issueTrack:  { height: 4, borderRadius: 2, backgroundColor: "rgba(255,255,255,0.06)", overflow: "hidden" },
