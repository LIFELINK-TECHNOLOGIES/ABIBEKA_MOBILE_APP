  ...
  return (
    <View style={{ marginBottom: 16 }}>
      ...
      <Animated.View style={[styles.inputWrap, { borderColor, backgroundColor: bgColor }]}>
        <Animated.Text style={[styles.inputIcon, { color: iconColor }]}>{icon}</Animated.Text>
        <TextInput
          value={value}
...
export default function SignUpScreen({ onSignInPress }: SignUpScreenProps) {
  ...
  const handleAccountTypeChange = (type: "user" | "org") => {
    ...
    Animated.sequence([
      Animated.timing(formShiftAnim, { toValue: 0, duration: 120, useNativeDriver: true }),
    ]).start(() => {
      setAccountType(type);
      Animated.timing(formShiftAnim, { toValue: 1, duration: 200, useNativeDriver: true }).start();
    });
  };
  ...
  const handleSignUp = () => {
    Animated.sequence([
      ...
      setUserRole(accountType === "user" ? "USER" : "ORG");
    });
  };
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <Animated.View style={[{ opacity: fadeAnim }, { marginBottom: 20 }]}>
              ...
              <Text style={styles.pageSub}>Configure identity encryption keys</Text>
            </Animated.View>

            {/* Type Switcher (Segmented Pill Layout) */}
            <Animated.View style={[styles.switchContainer, { opacity: fadeAnim }]}>
              <Pressable 
                style={[styles.switchTab, accountType === "user" && styles.switchTabActive]}
                onPress={() => handleAccountTypeChange("user")}
              >
                <Text style={[styles.switchText, accountType === "user" && styles.switchTextActive]}>
                  Individual
                </Text>
              </Pressable>
              <Pressable 
                style={[styles.switchTab, accountType === "org" && styles.switchTabActive]}
                onPress={() => handleAccountTypeChange("org")}
              >
                <Text style={[styles.switchText, accountType === "org" && styles.switchTextActive]}>
                  Organization
                </Text>
              </Pressable>
            </Animated.View>
            ...
            <Animated.View style={[styles.card, { opacity: fadeAnim }]}>
              ...
              <View style={styles.badge}>
                ...
                <Text style={styles.badgeText}>
                  {accountType === "user" ? "Personal Account Space" : "Enterprise Tenant Node"}
                </Text>
              </View>

              {/* Dynamic Animated Data Form */}
              <Animated.View style={{ opacity: formShiftAnim, transform: [{ translateY: contentTranslateY }] }}>
                {accountType === "user" ? (
                  <View>
                    <InputField label="Full Name" value={fullName} onChangeText={setFullName} icon="👤" />
                    <InputField label="Email Address" value={email} onChangeText={setEmail} icon="✉" />
                  </View>
                ) : (
                  <View>
                    <InputField label="Organization Name" value={orgName} onChangeText={setOrgName} icon="🏢" />
                    <InputField label="Corporate Business Email" value={email} onChangeText={setEmail} icon="✉" />
                    <InputField label="Tax ID / Registration Number" value={taxId} onChangeText={setTaxId} icon="📜" />
                  </View>
                )}

                <InputField label="Vault Access Password" value={password} onChangeText={setPassword} icon="⚿" secure />
              </Animated.View>

              {/* Register Callout Action */}
              <Animated.View style={{ marginTop: 12, transform: [{ scale: btnScale }] }}>
                <TouchableOpacity onPress={handleSignUp} activeOpacity={0.88} style={styles.btn}>
              ...
              <View style={styles.metaRow}>
                <View style={styles.metaItem}>
                  ...
                  <Text style={styles.metaText}>Zero Knowledge Architecture</Text>
                </View>
              </View>
            </Animated.View>
